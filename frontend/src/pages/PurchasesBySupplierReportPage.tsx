...
export function PurchasesBySupplierReportPage() {
  return (
    <RequirePermission
    ...
    >
      <PurchasesBySupplierReportContent />
    </RequirePermission>
  );
}

export function PurchasesBySupplierReportContent() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-bold text-gray-800">Reporte de Compras por Proveedor</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <SummaryCard label="Total Comprado" value={formatMoney(summary?.totalAmount ?? 0)} variant="money" />
        <SummaryCard label="Proveedores" value={String(summary?.totalRecords ?? 0)} variant="people" />
      </div>
      ...
      {!isLoading && !isError && (
        <>
          <Table<(typeof items)[number]>
            ...
            columns={[
              { header: 'Proveedor', accessor: (r) => r.supplierName },
              { header: 'N° de Compras', accessor: (r) => String(r.totalPurchases) },
              { header: 'Total Comprado', accessor: (r) => formatMoney(r.totalAmount) },
            ]}
            data={items}
          />
